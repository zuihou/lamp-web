<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input :placeholder="$t('table.msgs.title')" class="filter-item search-item" v-model="queryParams.title" />
      <el-input :placeholder="$t('table.msgs.content')" class="filter-item search-item" v-model="queryParams.content" />
      <el-date-picker
      ...
      <el-dropdown class="filter-item" trigger="click" v-has-any-permission="['msgs:add','msgs:delete','msgs:export']">
        <el-button>
          {{ $t('table.more') }}
    ...
    >
      ...
      <el-table-column :label="$t('table.msgs.title')" :show-overflow-tooltip="true" align="center" prop="templateId">
        <template slot-scope="scope">
          <span>{{ scope.row.title }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.msgs.content')" :show-overflow-tooltip="true" align="center" prop="receiver">
        <template slot-scope="scope">
          <span>{{ scope.row.content }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.msgs.author')" :show-overflow-tooltip="true" align="center" width="100px">
        <template slot-scope="scope">
          <span>{{ scope.row.author }}</span>
        </template>
      </el-table-column>
      <el-table-column :filter-multiple="false" :filters="bizTypeFilters" :label="$t('table.msgs.bizType')" :show-overflow-tooltip="true" align="center" column-key="bizType" width="100px">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          <span>
            <el-tag :type="scope.row.isRead?'danger':'success'" slot>{{ scope.row.isRead ? '是' : '否' }}</el-tag>
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.operation')" align="center" class-name="small-padding fixed-width" width="100px">
        <template slot-scope="{row}">
          <i @click="view(row)" class="el-icon-view table-operation" style="color: #2db7f5;" v-hasPermission="['msgs:view']" />
          <i @click="singleDelete(row)" class="el-icon-delete table-operation" style="color: #f50;" v-hasPermission="['msgs:delete']" />
          <el-link class="no-perm" v-has-no-permission="['msgs:view','msgs:delete']">{{ $t('tips.noPermission') }}</el-link>
        </template>
      </el-table-column>
    </el-table>
